...
local mod_path	= 'D:/MODZ_stuff/NoMansSky/Published/Technology Icons/_MOD.lMonk.Technology Icons'
...
local gc_tech = { meta = {name='Table'} }
for id, icon in pairs(tech_icons) do
	gc_tech[#gc_tech+1] = {
		meta = {name='Table', value='GcTechnology', _id=id},
		ID = id,
		Icon = {
...
os.execute(
	('robocopy "%s" "%s\\%s" /S /XF %s'):format(
		lsh('D:/MODZ_stuff/NoMansSky/Sources/_Textures_mod_source/textures/ui/frontend/icons/technology'),
		lsh(mod_path),
		lsh(dds_trg..'/technology'),
		'_MY*' -- exclude files
	)
)

print('Technology Icons Mod Created.')
