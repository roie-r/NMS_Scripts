...
------------------------------------------------------------------------------------
local desc = [[
  ** Scale biome props & increased LOD **
  The script matches 'tags' from Scale_tags table to the model scene file names
  & full path(!), then multiples the tags' values with the properties in
  Object_Spawn_Prop table.
...
local LOD_Increase = 1.5 -- 0 to skip
...
-- unless one contins u=true, then return the specific tag
...
local function SetTheScales(biomeObjects, globalTags, biomeTags)
	...
	for _,src in pairs(biomeObjects) do
		...
		if LOD_Increase > 0 then
			local lod_change = {
				REPLACE_TYPE 		= 'ALL',
				INTEGER_TO_FLOAT	= 'PRESERVE',
				MATH_OPERATION 		= '*',
				PRECEDING_KEY_WORDS = 'LodDistances',
				VALUE_CHANGE_TABLE 	= { {'IGNORE', LOD_Increase} }
			}
			table.insert(mods_tbl.EXML_CHANGE_TABLE, lod_change)
		end
		-- LOD increase --
...
-- naming names
local function LodName(p)
	if p > 0 then return ' & '..p..'xLOD' end
	return ''
end
...
NMS_MOD_DEFINITION_CONTAINER = {
	MOD_FILENAME 		= '_META Big props'..LodName(LOD_Increase)..'.pak',
	MOD_AUTHOR			= 'lMonk',
	NMS_VERSION			= 3.84,
